...
import { Check, FileClock, Plus, Trash2, X } from "lucide-react";
import { apiFetch } from "@/lib/http";
import { formatDate, STATUS_LABEL, cn } from "@/lib/utils";
import { Card, EmptyState, PageHeader, StatusBadge } from "@/components/ui";
import { Modal } from "@/components/Modal";

interface ExtensionRequest {
  id: number; currentEndDate: string; requestedEndDate: string; reason: string;
  status: "PENDING" | "APPROVED" | "REJECTED"; reviewNote: string | null;
  user: { fullName: string; department: { name: string } | null };
  reviewedBy: { fullName: string } | null;
}

export function ContractExtensionClient({ canRequest, canApprove }: { canRequest: boolean; canApprove: boolean }) {
  const [scope, setScope] = useState<"mine" | "all">(canApprove && !canRequest ? "all" : "mine");
  const [list, setList] = useState<ExtensionRequest[]>([]);
  ...
  const load = useCallback(async () => {
    setList(await apiFetch<ExtensionRequest[]>(`/api/contract-extensions?scope=${scope}`));
  }, [scope]);
  useEffect(() => { load(); }, [load]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError("");
    try {
      await apiFetch("/api/contract-extensions", { method: "POST", body: JSON.stringify(form) });
      setOpen(false);
  ...
  async function review(id: number, action: "approve" | "reject") {
    ...
    try {
      await apiFetch(`/api/contract-extensions/${id}`, { method: "PATCH", body: JSON.stringify({ action, reviewNote: note }) });
      await load();
    } catch (err) {
  ...
  async function cancel(id: number) {
    ...
    await apiFetch(`/api/contract-extensions/${id}`, { method: "DELETE" });
    await load();
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={canApprove ? "Tinjau dan setujui pengajuan perpanjangan kontrak/magang karyawan." : "Ajukan perpanjangan periode kontrak/magang Anda dan pantau statusnya."}
        action={canRequest && <button className="btn-primary" onClick={() => setOpen(true)}><Plus className="h-4 w-4" /> Ajukan Perpanjangan</button>}
      />
      ...
      {!canRequest && !canApprove ? (
        <EmptyState title="Tidak ada yang bisa ditampilkan" subtitle="Halaman ini untuk karyawan berstatus Kontrak/Magang, atau HR/CEO yang meninjau pengajuan." icon={<FileClock className="h-10 w-10" />} />
      ) : list.length === 0 ? (
        <EmptyState title="Belum ada pengajuan perpanjangan" icon={<FileClock className="h-10 w-10" />} />
      ) : (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="font-semibold text-slate-800 dark:text-slate-100">Perpanjangan Kontrak</p>
                    ...
                    <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{formatDate(l.currentEndDate)} → {formatDate(l.requestedEndDate)}</p>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Ajukan Perpanjangan Kontrak">
        <form onSubmit={submit} className="space-y-4">
          <div>
            <label className="label">Tanggal Perpanjangan Baru</label>
            <input type="date" className="input" value={form.requestedEndDate} onChange={(e) => setForm((f) => ({ ...f, requestedEndDate: e.target.value }))} required />
          </div>
          <div>
